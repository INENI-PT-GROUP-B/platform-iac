...
- `roles/secretmanager.admin` on the project for the operator. Phases 1a and 1b both write new `SecretVersion`s (`secretmanager.versions.add`) and read the latest version back (`secretmanager.versions.access`) to short-circuit a re-run when the payload is unchanged. The read permission is **not** part of `roles/editor`, so without this grant the affected phase aborts with a clear `PERMISSION_DENIED` and the hint to grant the role:
...
- **Optional:** Grafana admin credentials in `GRAFANA_ADMIN_PASSWORD` (and `GRAFANA_ADMIN_USER`, defaulting to `admin`) in `bootstrap/bootstrap.env` for Phase 1b to seed the `grafana-admin` entry in Google Secret Manager. ESO syncs the JSON payload into the `monitoring` namespace as the K8s Secret `grafana-admin-credentials`, consumed by the kube-prometheus-stack Grafana via `grafana.admin.existingSecret` (see `INENI-PT-GROUP-B/platform-gitops` `monitoring/grafana-admin-externalsecret.yaml`).

  When `GRAFANA_ADMIN_PASSWORD` is unset, Phase 1b logs a warn and continues. Day-1 cluster bring-up still succeeds; the monitoring stack comes up but Grafana stays unavailable (Pod waiting for the admin Secret) until the password is set and `bootstrap.sh` re-runs. The phase is idempotent: a re-run with the same password is a no-op, a re-run with a rotated password adds exactly one new SecretVersion. Pick a strong random password — the chart-default `prom-operator` is unacceptable for an internet-exposed host.
...
- **Phase 1b — Grafana admin seed:** renders the JSON object `{"admin-user": "...", "admin-password": "..."}` from `GRAFANA_ADMIN_USER` / `GRAFANA_ADMIN_PASSWORD` and writes it into the `grafana-admin` entry in Google Secret Manager (same idempotent create-if-absent shape as Phase 1a). The kube-prometheus-stack Application (`platform-gitops` S4-01) wires an `ExternalSecret` that materialises this entry into the `monitoring` namespace as the `grafana-admin-credentials` K8s Secret. Skipped with a warn when `GRAFANA_ADMIN_PASSWORD` is unset.
